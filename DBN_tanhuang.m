...
%得到输入和输出的 evidences
evidence{1,T-2} = 0;%输入u 
evidence{3,T-2} = 0;%输出y  
evidence{1,T-3} = 0;%输入u 
evidence{3,T-3} = 0;%输出y  
u = 0;
...
for i = 1:100
    ...
    X_t = X_t1;
    evidence{3,t+1} = Y_t1;
    evidence{3,t+2} = 10;   %目标值
    evidence{3, t+3} = 10;   %目标值
    evidence{3, t+4} = 10;   %目标值
    evidence{3, t+5} = 10;   %目标值
    %DBN网络推理计算input
